...
const LineChartClient = () => {
  ...
  useEffect(() => {
    const loadUsers = async () => {
      api("/newInfos/getMeasuresByUserId/" + userId, {
        method: "GET",
      })
        .then((data) => {
          data.json().then((json) => {
  ...
  var data = {
    ...
    datasets: [
      {
        label: "Massa Gorda",
        data: measures?.map((x) => x.fatMassPercentage),
        tension: 0.1,
        backgroundColor: [
          "rgba(255, 99, 132, 0.2)",
          "rgba(54, 162, 235, 0.2)",
      ...
      {
        label: "Massa Magra",
        data: measures?.map((x) => x.leanMassPercentage),
        tension: 0.1,
        backgroundColor: [
          "rgba(255, 99, 132, 0.2)",
          "rgba(54, 162, 235, 0.2)",
  ...
  var options = {
    ...
    plugins: {
      title: {
        display: true,
        text: "Evolução do Percentual de Massa Gorda e Magra",
      },
      lengend: {
        display: true,
        text: "test",
